...
def dispatch_msg(manager: MemoryMain, msg: dict, peer_id: int):
    ...
    if msg_type == CHOKE_TYPE:
        return manager.handle_choke(peer_id)
    elif msg_type == UNCHOKE_TYPE:
        return manager.handle_unchoke(peer_id)
    elif msg_type == INTERESTED_TYPE:
        return manager.handle_interested(peer_id)
    elif msg_type == NOT_INTERESTED_TYPE:
        return manager.handle_not_interested(peer_id)
    elif msg_type == HAVE_TYPE:
        return manager.handle_have(peer_id, msg["piece_index"])
    elif msg_type == BITFIELD_TYPE:
        return manager.handle_bitfield(peer_id, msg["bitfield"])
    elif msg_type == REQUEST_TYPE:
        return manager.handle_request(peer_id, msg["piece_index"])
    elif msg_type == PIECE_TYPE:
        return manager.handle_piece(peer_id, msg["piece_index"], msg["data"])
